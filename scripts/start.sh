...
if [[ "$NEURON_TYPE" == "validator" ]]; then
    pm2 start "$HOME/.local/bin/uv run neurons/validators/validator.py --netuid $NETUID --subtensor.network $NETWORK --wallet.name $COLDKEY --wallet.hotkey $HOTKEY --logging.info --axon.port $AXON_PORT" --name webgenie_validator
